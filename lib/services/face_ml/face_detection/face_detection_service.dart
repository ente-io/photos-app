...
class FaceDetection {
  ...
  List<FaceDetectionAbsolute> predict(Uint8List imageData) {
    ...
    _logger.info('Input of shape ${input.shape}');
    _logger.info(
      'Outputs: of shape ${outputs[0]?.shape} and ${outputs[1]?.shape}',
    );
    ...
    _logger.info(
      'interpreter.run is finished, in ${secondStopwatch.elapsedMilliseconds} ms',
    );
