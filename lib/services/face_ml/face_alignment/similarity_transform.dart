...
class SimilarityTransform {
  ...
  ///
  /// Returns the warped face in the specified width and height, in [Double3DInputMatrix], in range [-1,1].
  ///
  /// Runs efficiently in about 3-9 ms after initial warm-up.
  Double3DInputMatrix warpAffineToMatrix({
    required image_lib.Image inputImage,
    required Matrix transformationMatrix,
    int width = 112,
    int height = 112,
  }) {
    final Double3DInputMatrix outputMatrix = List.generate(
      height,
      (y) => List.generate(
        width,
        (_) => List.filled(3, 0.0),
    ...
    try {
      ...
      _logger.severe("Exception in `warpAffineToMatrix`:", e, s);
      rethrow;
    }
